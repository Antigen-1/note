...
◊p{我们已经讨论了编程的基本元素，但是我们仍未曾了解编程的常用策略和模式。}

◊p{编程就是在策划一系列动作的执行方针，因此对各个动作结果的预测就非常关键。}

◊p{
一个函数（◊i{procedure}）是一个计算过程局部演化（◊i{local evolution}）的模式。

它规定了过程的每一个阶段如何建立在前一个阶段之上。

我们要能够清楚一个局部演化已知的过程的全局的（◊i{global}）表现。

这很困难，但是我们可以至少尝试描述几种过程演化的典型模式。}

◊p{在这一节里我们会考察简单函数生成的过程的通用的形态，以及这些过程消耗时间和空间这些计算资源的比率。}

◊h3{线性递归和迭代}

◊p{线性递归和迭代看上去是等价的——它们接受同样的定义域，经历了同样的步骤，得到了同样的结果。}

◊p{然而，从另一方面讲，我们认为二者的「形状」是大不相同的。}

◊p{前者展现的是一种先展开后合并的方式。展开过程产生了一系列延迟操作（◊i{deferred operation}），合并过程则是真正进行计算。}

◊p{这种以延迟计算为特征的过程称之为递归过程（◊i{recursive process}）。}

◊p{
执行这种过程要求解释器追踪以后可能要执行的操作的信息。

如果步骤数、延迟计算链的长度、信息的量呈线性增加则为线性递归过程（◊i{linear recursive process}）。}

◊p{与之相反，后者没有这种增大和缩减——在每一步都只需要追踪每一个参数对应的状态信息（当然这是我们而非解释器的工作）。}

◊p{在一个迭代过程（◊i{iterative process}）中，储存状态信息的变量被称为状态变量（◊i{state variable}）。

此外还有一个固定的规则描述随着迭代进行如何更新状态变量，以及一个（可选的）终点测试决定过程何时终止。}

◊p{迭代过程执行的步数呈线性增加时，又称线性迭代过程（◊i{linear iterative process}）。}

◊p{这两个过程之间的差别也可以这样描述：在递归过程中由解释器维护的信息，在迭代过程中改由固定的状态变量保存了，并随着过程演化更新了。

因此当解释器需要真正计算时，前者使用的是解释器累积下来的信息，而后者仅需要最后的状态变量。}

◊p{另外，我们不能混淆递归过程和递归函数——当我们说一个函数是递归的的时候，我们是想表示一个函数的定义引用了它自身。

而当我们说一个过程是线性递归的时候，只是在描述这个过程是如何演化的，而与语法无关。}

◊p{过程和函数如此容易混淆还有另一个原因：大部分语言将递归函数的执行实现为内存的消耗随着函数调用的增加而增加。

甚至在其描述的过程是迭代的时候也不例外。

因此这些语言只能诉诸于特殊目的的“循环结构”。}

◊p{scheme则没有这种缺陷，它永远在恒定空间中执行迭代过程，即使它被描述为递归函数。

有这种特性的实现被称为尾递归的（◊i{tail-recursive}）。

有了尾递归的实现，特殊的迭代结构就只是语法糖而已了。}

◊h3{树递归}

◊p{树递归也有递归和迭代两种实现。}

◊p{前者步数与节点数成正比，所需的空间与输入成正比。}

◊p{后者步数与输入成正比，所需空间近乎恒定。}

◊p{前者的优点是直观，以及处理分层的复合数据结构时方便，缺点是可能存在大量重复计算。后者恰恰相反。}

◊p{基于此，设计一个更加聪明的编译器，将树递归过程转化为效率更高的其他过程，以同时利用两种过程的优点的想法，就非常自然了。}
