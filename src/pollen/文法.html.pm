...
◊lst{
ul
[NCE4.8]◊i{So, ask businesses on both sides of the Atlantic, why have two lots of tests where one would do?}那么「so[conj](informal)（引出评论或问题）」，大西洋两岸的公司会问，为什么一套测试足够「do[v]be enough」还要通过两套「lot[C]（一）组，群，批，套」测试？
}
